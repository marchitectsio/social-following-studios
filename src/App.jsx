...
function PartnerMarquee({ tone = "light" }) {
  const fades = {
    light: {
  ...
  const chip = {
    ...
    clean: "border-stone-900/10 bg-white/70 text-stone-900",
  };
      ...
      <div className="flex w-max animate-marquee whitespace-nowrap">
        {[...PARTNERS, ...PARTNERS, ...PARTNERS].map((name, idx) => (
          <div key={`${name}-${idx}`} className="px-3">
            <div className={cx(
              "inline-flex items-center rounded-full border px-5 py-2 text-[11px] md:text-xs font-black tracking-[0.25em] uppercase shadow-md transition-all duration-500 hover:-translate-y-0.5",
              chip[tone]
            )}>
              {name}
            </div>
    ...
    class TouchTexture {
      ...
      update() {
        ...
        for (let i = this.trail.length - 1; i >= 0; i--) {
          ...
          this.ctx.shadowColor = `rgba(${((point.vx + 1) / 2) * 255}, ${((point.vy + 1) / 2) * 255}, ${
            intensity * 255
          }, ${0.35 * intensity})`;
...
function Background() {
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <LiquidBackground />
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(245,242,234,0.10),transparent_65%)]" />
      <Grid />
    </div>
  );
}
      ...
      <section className="relative overflow-hidden rounded-[3rem] border-2 border-stone-900/15 bg-stone-950 p-8 md:p-24 shadow-2xl text-left">
        ...
        <div className="relative mx-auto text-left max-w-5xl">
          ...
          <div className="mt-14 flex flex-wrap gap-6">
            <Button href="https://cal.com/rashida-knox" variant="primary" size="large">GET STARTED</Button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-12">
      <section className="rounded-[3rem] border-2 border-stone-900/10 bg-white/80 backdrop-blur-xl p-12 md:p-16 shadow-2xl text-left">
        <SectionHead eyebrow="SYSTEM" title="The System" desc="Four conversion pillars working as one system." right={<Button href="https://cal.com/rashida-knox" variant="primary">GET STARTED</Button>} />
      </section>
      ...
      <section className="pt-16 md:pt-20 text-left">
        <div className="flex items-center justify-between gap-6">
          <div className="text-[11px] md:text-xs font-black tracking-[0.35em] text-emerald-700 uppercase leading-none">Trusted by industry leaders</div>
          <div className="text-[10px] md:text-xs font-black tracking-[0.25em] text-stone-600 uppercase">Proof layer</div>
        </div>
        <div className="relative mt-6 overflow-hidden">
          <PartnerMarquee tone="light" />
        </div>
      </section>
...
function Shell({ route, children }) {
  ...
  return (
    const shellBg = isHome ? "bg-transparent" : "bg-[#F5F2EA]";

  return (
    <div className={cx("min-h-screen text-stone-900 overflow-x-hidden selection:bg-emerald-600 selection:text-white", shellBg)}>
      {isHome && <Background />}
      <CustomCursor />
      ...
      <header className="sticky top-0 z-50 border-b-2 border-stone-900/10 bg-[#F5F2EA]/85 backdrop-blur-2xl shadow-sm text-left">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 text-left">
          <a href="#/" className="flex items-center gap-4 group">
            <LogoPlaceholder className="h-14 md:h-20 w-auto" />
          </a>

          <nav className="hidden md:flex items-center gap-1 text-left">
            {NAV.map((item) => (
              <a key={item.href} href={item.href} className={cx("rounded-2xl px-5 py-3 text-base md:text-lg font-black tracking-wide uppercase transition-all duration-300", active(item.href) ? "bg-stone-950 text-white shadow-2xl" : "text-stone-900 hover:text-stone-950 hover:bg-white/50")}>{item.label}</a>
            ))}
          </nav>

          <div className="flex items-center gap-3">
            <Button href="https://cal.com/rashida-knox" variant="primary" size="default">GET STARTED</Button>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t-2 border-stone-900/10 bg-[#F5F2EA]/95 backdrop-blur-2xl">
            <nav className="mx-auto max-w-7xl px-6 py-4 flex flex-col gap-2">
              {NAV.map((item) => (
                <a key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className={cx("rounded-2xl px-5 py-4 text-lg font-black tracking-wide uppercase transition-all duration-300 text-center", active(item.href) ? "bg-stone-950 text-white shadow-2xl" : "text-stone-900 hover:text-stone-950 hover:bg-white/50")}>{item.label}</a>
              ))}
            </nav>
          </div>
        )}
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-10 text-left">
            <div className="flex items-center">
              <LogoPlaceholder className="h-16 md:h-20 w-auto opacity-70 hover:opacity-100 transition-all duration-500" />
            </div>
